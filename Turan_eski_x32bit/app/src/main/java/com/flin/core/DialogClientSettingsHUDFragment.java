...
public class DialogClientSettingsHUDFragment extends Fragment implements ISaveableFragment {
    ...
    private void setSeekBarListeners()
    {
        ...
        for(int i = -DialogClientSettings.mSettingsHudCountOffsetLeft; i < DialogClientSettings.mSettingsHudCount; i++)
        {
        ...
        for(int i = 0; i < 4; i++)
        {
            ...
            if(i == 3){
                String widgetSensitivity = "custom_widget_sensitivity_" + i;
                int resIDSensitivity = mContext.getResources().getIdentifier(widgetSensitivity, "id", mContext.getPackageName());
                SeekBar barSensitivity = mRootView.findViewById(resIDSensitivity);
                if(barSensitivity != null) {
                    barSensitivity.setOnSeekBarChangeListener(mListenerSeekBars);
                }
            }
    ...
    public void getValues() {
        ...
        for(int i = 0; i < 4; i++)
        {
            ...
            if(i == 3){
                ...
                int resIDSensitivity = mContext.getResources().getIdentifier(widgetSensitivity, "id", mContext.getPackageName());
                SeekBar barSensitivity = mRootView.findViewById(resIDSensitivity);
               // mContext.getNativeWidgetSensitivity(i);
                if(barSensitivity != null) {
                   // barSensitivity.setProgress(mContext.getNativeWidgetSensitivity(i));
                }
            }
    ...
    public void passValuesToNative()
    {
        ...
        for(int i = 0; i < 4; i++)
        {
            ...
            if(i == 3){
                ...
                int resIDSensitivity = mContext.getResources().getIdentifier(widgetSensitivity, "id", mContext.getPackageName());
                SeekBar barSensitivity = mRootView.findViewById(resIDSensitivity);
              //  mContext.setNativeWidgetSensitivity(i, barSensitivity.getProgress());
            }
