...
public class WarMedia extends WarGamepad
{
    ...
    public String GetGameBaseDirectory() {
        if (Environment.getExternalStorageState().equals("mounted"))
        {
            try
            {
                ...
                System.out.println("Mihail absolutePath: "+absolutePath);
                this.baseDirectoryRoot = absolutePath.substring(0, absolutePath.indexOf("/Android"));
                System.out.println("Mihail: "+this.baseDirectoryRoot);
    ...
    public void onCreate(Bundle bundle)
    {
        ...
        System.out.println("Mihail2: "+this.baseDirectoryRoot);
        NvUtil.getInstance().setActivity(this);
        System.out.println("Mihail STORAGE_ROOT: "+this.baseDirectory);
        System.out.println("Mihail baseDirectoryRoot: "+this.baseDirectoryRoot);
        // NvUtil.getInstance().setAppLocalValue("STORAGE_ROOT", this.baseDirectory);
        // NvUtil.getInstance().setAppLocalValue("STORAGE_ROOT", Environment.getExternalStorageDirectory() + "/FlinOnline/files/");
        //   NvUtil.getInstance().setAppLocalValue("STORAGE_ROOT","/storage/extSdCard/FlinOnline/files/");
        NvUtil.getInstance().setAppLocalValue("STORAGE_ROOT_BASE", this.baseDirectoryRoot);
        if(PublicInfo.GTAmods == 1){
            int mod_id = 0;
            try {
                mod_id = CheckLoadingFiles.getModsID();
                NvUtil.getInstance().setAppLocalValue("STORAGE_ROOT",Environment.getExternalStorageDirectory() + "/Android/data/"+PublicInfo.checkReleasePackageName+"/mods/"+mod_id+"/files/");
            } catch (IOException e) {
                e.printStackTrace();

                NvUtil.getInstance().setAppLocalValue("STORAGE_ROOT", this.baseDirectory);
                //  NvUtil.getInstance().setAppLocalValue("STORAGE_ROOT", Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS) + "/FlinOnline/files/");
            }
        }
        else {
            NvUtil.getInstance().setAppLocalValue("STORAGE_ROOT", this.baseDirectory);
            ...
            //NvUtil.getInstance().setAppLocalValue("STORAGE_ROOT", Environment.getExternalStorageDirectory() + "/Download/FlinOnlines/files/");
