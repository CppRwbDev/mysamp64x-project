...
extern "C" {
	JNIEXPORT void JNICALL Java_com_turan_game_SAMP_initializeSAMP(JNIEnv *pEnv, jobject thiz)
	{
		pJavaWrapper = new CJavaWrapper(pEnv, thiz);

	}
	JNIEXPORT void JNICALL Java_com_turan_game_SAMP_onInputEnd(JNIEnv *pEnv, jobject thiz, jbyteArray str)
	{
		if(pUI)
		{
	...
	JNIEXPORT void JNICALL Java_com_turan_game_SAMP_onEventBackPressed(JNIEnv *pEnv, jobject thiz)
	{
		if(pSettings)
		{
	...
	JNIEXPORT void JNICALL Java_com_turan_game_ui_dialog_DialogManager_sendDialogResponse(JNIEnv* pEnv, jobject thiz, jint i3, jint i, jint i2, jbyteArray str)
	{
...
Java_com_turan_game_ui_Hud_clickCameraMode(JNIEnv *env, jobject thiz) {
//
    if(!pNetGame)return;
...
extern "C"
JNIEXPORT void JNICALL
Java_com_turan_game_ui_Hud_sendAltKey(JNIEnv *env, jobject thiz) {
    LocalPlayerKeys.bKeys[ePadKeys::KEY_WALK] = true;
}

extern "C"
JNIEXPORT void JNICALL
Java_com_turan_game_ui_Hud_sendCtrlKey(JNIEnv *env, jobject thiz) {
    LocalPlayerKeys.bKeys[ePadKeys::KEY_CTRL_BACK] = true;
}

extern "C"
JNIEXPORT void JNICALL
Java_com_turan_game_ui_Hud_sendTabKey(JNIEnv *env, jobject thiz) {
    if(pUI) {
        PlayerTabList* tab = pUI->playertablist();
        if(tab) {
            if(tab->visible()) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_turan_game_ui_Hud_SetRadarBgPos(JNIEnv *env, jobject thiz, jfloat x1, jfloat y1,
                                               jfloat x2, jfloat y2) {
    radarBgPos1.x = x1;
    radarBgPos1.y = y1;
    radarBgPos2.x = x2;
...
extern void SwitchNextWeapon();

JNIEXPORT void JNICALL
Java_com_turan_game_ui_Hud_cycleWeapon(JNIEnv *env, jobject thiz) {
    FLog("Java_com_turan_game_ui_Hud_cycleWeapon called");
    SwitchNextWeapon();
}

JNIEXPORT void JNICALL
Java_com_turan_game_ui_Hud_fireWeapon(JNIEnv *env, jobject thiz) {
