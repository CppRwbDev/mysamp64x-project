...
CWeapon* CPlayerPed::GetCurrentWeaponSlot()
{
    if (m_pPed)
    {
        if (m_pPed->m_nActiveWeaponSlot < 0 || m_pPed->m_nActiveWeaponSlot >= 13)
        {
             // FLog("Error: Invalid Weapon Slot: %d", m_pPed->m_nActiveWeaponSlot);
             return &m_pPed->m_aWeapons[0];
        }
...
void CPlayerPed::FireInstant()
{
    ...
    if (!m_pPed) {
        LOGI("[WEAPON] FireInstant: m_pPed is NULL, returning");
        return;
    }
    
    if (!m_dwGTAId) {
        LOGI("[WEAPON] FireInstant: m_dwGTAId is 0, returning");
        return;
    }
    
    if (!GamePool_Ped_GetAt(m_dwGTAId)) {
        LOGI("[WEAPON] FireInstant: Ped not in pool, returning");
        return;
    }
    ...
    CVector vecBonePos = {0.0f, 0.0f, 0.0f};
    ...
    else
    {
        ...
        if (pSlot)
        {
            LOGI("[WEAPON] FireInstant: Type: %d, Ammo: %d, State: %d", pSlot->dwType, pSlot->dwAmmo, pSlot->dwState);
            //if (pSlot->dwAmmo == 0) return; // Debug: check if ammo is issue
        }
        else
        {
            LOGI("[WEAPON] FireInstant: pSlot is NULL");
        }
...
void CPlayerPed::GetWeaponInfoForFire(bool bLeftWrist, CVector* vecBonePos, CVector* vecOut)
{
    ...
    if (pFireOffset && vecBonePos && vecOut) {
        ...
        LOGI("[WEAPON] FireOffset: %f %f %f", pFireOffset->x, pFireOffset->y, pFireOffset->z);
