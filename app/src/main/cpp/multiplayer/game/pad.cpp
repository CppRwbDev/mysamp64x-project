...
uint32_t CPad__GetWeapon_hook(uintptr_t thiz, uintptr_t ped, bool unk)
{
    ...
    else
    {
        ...
        if (pGame) {
            CPlayerPed* pPlayerPed = pGame->FindPlayerPed();
            if (pPlayerPed) {
                static long long lastFireTime = 0;
                ...
                if (value) {
                    ...
                    int weaponID = pPlayerPed->GetCurrentWeapon();
                    int fireRate = 180;

                    switch(weaponID) {
                        case 22: fireRate = 180; break;
                        case 23: fireRate = 180; break;
                        case 24: fireRate = 600; break; // Deagle
                        case 25: fireRate = 800; break; // Shotgun
                        case 26: fireRate = 250; break;
                        case 27: fireRate = 350; break;
                        case 28: fireRate = 70; break; // Uzi
                        case 29: fireRate = 100; break; // MP5
                        case 30: fireRate = 110; break; // AK
                        case 31: fireRate = 110; break; // M4
                        case 32: fireRate = 70; break; // Tec9
                        case 33: fireRate = 900; break; // Rifle
                        case 34: fireRate = 1100; break; // Sniper
                        case 38: fireRate = 50; break; // Minigun
                    }

                    if ((currentTime - lastFireTime) > fireRate) {
                         CWeapon* pWeapon = pPlayerPed->GetCurrentWeaponSlot();
                         if (pWeapon) {
                             if (pWeapon->dwState == eWeaponState::WEAPONSTATE_RELOADING) {
                                 // Do nothing
                             }
                             else if (pWeapon->dwAmmoInClip > 0) {
                                 pPlayerPed->FireInstant();

                                 if (pWeapon->dwAmmoInClip > 0) pWeapon->dwAmmoInClip--;
                                 if (pWeapon->dwAmmo > 0) pWeapon->dwAmmo--;

                                 if (pWeapon->dwAmmoInClip == 0 && pWeapon->dwAmmo > 0) {
                                     pWeapon->dwState = eWeaponState::WEAPONSTATE_RELOADING;
                                 }

                                 lastFireTime = currentTime;
                             } else if (pWeapon->dwAmmo > 0) {
                                  // Auto Reload
                                  pWeapon->dwState = eWeaponState::WEAPONSTATE_RELOADING;
                             }
                         }
